...
<div x-data="paketPage()" class="space-y-5 fade-in-up">
    <div class="flex items-center justify-between">
        <p class="text-sm text-slate-400">Kelola paket kursus barbershop</p>
        <button @click="openTambah()" class="inline-flex items-center gap-2 rounded-xl bg-emerald-500 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-emerald-500/25 hover:bg-emerald-600">
    ...
    <div class="rounded-2xl border border-slate-700/50 bg-[#1C2434] shadow-xl overflow-hidden">
                ...
                <tbody>
                    @forelse($paketKursus as $pkg)
                        <tr class="border-b border-slate-700/50 table-row-hover">
                            <td class="px-5 py-4 font-semibold text-white">{{ $pkg->nama_paket }}</td>
                            <td class="px-5 py-4 text-slate-400 max-w-[200px] truncate">{{ $pkg->deskripsi ?? '-' }}</td>
                            ...
                            <td class="px-5 py-4 text-slate-300 whitespace-nowrap">Rp {{ number_format($pkg->harga, 0, ',', '.') }}</td>
                            <td class="px-5 py-4 text-slate-300">{{ $pkg->lama_pelatihan }}</td>
        ...
        @if($paketKursus->hasPages())<div class="border-t border-slate-700/50 px-5 py-4">{{ $paketKursus->links() }}</div>@endif
    </div>
...
<div x-data x-show="$store.paketModal.show" x-cloak
    ...
    <div class="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl border border-slate-700 bg-[#1C2434] p-8 shadow-2xl [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
        ...
        <div class="flex items-center justify-between mb-6">
            <h3 class="text-xl font-bold text-white" x-text="$store.paketModal.isEdit ? 'Edit Paket' : 'Tambah Paket'"></h3>
            <button @click="$store.paketModal.close()" class="rounded-lg p-1.5 text-slate-400 hover:bg-slate-700 hover:text-white transition-colors">
        ...
        <form :action="$store.paketModal.formAction" method="POST" class="space-y-5">
            ...
            <div>
                <label class="mb-1.5 block text-sm font-medium text-slate-400">Nama Paket</label>
                <input type="text" name="nama_paket" x-model="$store.paketModal.form.nama_paket" required
                       class="w-full rounded-xl border border-slate-600/50 bg-[#111827] px-4 py-3 text-sm text-white placeholder-slate-500 outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-colors">
            </div>

            {{-- Baris 2: Deskripsi (Full Width) --}}
            <div>
                <label class="mb-1.5 block text-sm font-medium text-slate-400">Deskripsi</label>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-slate-400">Kategori</label>
                ...
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-slate-400">Harga</label>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-slate-400">Lama Pelatihan</label>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-6 pt-1">
                        <label class="flex items-center gap-2 cursor-pointer group">
                            ...
                            <span class="text-sm text-slate-300 group-hover:text-white transition-colors">Aktif</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer group">
                            <input type="radio" name="status" value="tidak_aktif" x-model="$store.paketModal.form.status"
                                   class="h-4 w-4 border-2 border-slate-500 bg-transparent text-emerald-500 focus:ring-emerald-500 focus:ring-offset-0 focus:ring-offset-transparent">
                            <span class="text-sm text-slate-300 group-hover:text-white transition-colors">Tidak Aktif</span>
                        </label>
                    </div>
                </div>
            </div>
...
function paketPage() {
    return {
        ...
        hapus(id, n) {
            Swal.fire({ title:'Hapus paket?', text:'Paket "'+n+'" akan dihapus.', icon:'warning', showCancelButton:true, confirmButtonColor:'#EF4444', cancelButtonColor:'#475569', confirmButtonText:'Ya, Hapus!', cancelButtonText:'Batal', background:'#1C2434', color:'#e2e8f0' }).then(r => {
