...
<div class="space-y-5 fade-in-up">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <form method="GET" action="{{ route('laporan.peserta') }}" class="flex flex-wrap items-center gap-3">
            <div class="relative">
                ...
                <input type="text" name="search" value="{{ $search }}" placeholder="Cari nama atau NIK..." class="rounded-xl border border-slate-600/50 bg-[#253044] py-2.5 pl-10 pr-4 text-sm text-white placeholder-slate-500 outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500">
            </div>
            <select name="status" class="rounded-xl border border-slate-600/50 bg-[#253044] px-4 py-2.5 text-sm text-white outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500">
                <option value="">Semua Status</option>
                @foreach(['aktif','selesai'] as $s)
                    <option value="{{ $s }}" {{ $status==$s?'selected':'' }}>{{ ucfirst($s) }}</option>
                @endforeach
            </select>
            <button type="submit" class="rounded-xl bg-emerald-500 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-600">Filter</button>
        </form>
        <a href="{{ route('laporan.peserta.export-pdf', request()->only(['search', 'status'])) }}" target="_blank" class="inline-flex items-center gap-2 rounded-xl bg-red-500/15 border border-red-500/30 px-4 py-2 text-sm font-medium text-red-400 hover:bg-red-500/25">
    ...
    <div class="rounded-2xl border border-slate-700/50 bg-[#1C2434] shadow-xl overflow-hidden">
            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-slate-700/50 table-row-hover">
                        <td class="px-5 py-4 font-semibold text-white">{{ $p->nama }}</td>
                        ...
                        <td class="px-5 py-4 text-slate-400 max-w-[200px] truncate">{{ $p->alamat }}</td>
                    </tr>
                    @empty
        ...
        @if($peserta->hasPages())<div class="border-t border-slate-700/50 px-5 py-4">{{ $peserta->withQueryString()->links() }}</div>@endif
    </div>
</div>
@endsection
