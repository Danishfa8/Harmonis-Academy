...
<div class="space-y-5 fade-in-up">
    ...
    <div class="rounded-2xl border border-slate-700/50 bg-[#1C2434] p-6 shadow-xl">
        <h3 class="mb-4 text-lg font-semibold text-white">Total Pembayaran Bulanan</h3>
        <div class="h-72">
    ...
    <form method="GET" action="{{ route('riwayat-pembayaran.index') }}" class="flex items-center gap-3">
        <select name="peserta_id" class="rounded-xl border border-slate-600/50 bg-[#253044] px-4 py-2.5 text-sm text-white outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500">
            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-slate-700/50 table-row-hover">
                        ...
                        <td class="px-5 py-4 text-slate-400 max-w-[200px] truncate">{{ $pay->keterangan ?? '-' }}</td>
                    </tr>
                    @empty
                    <tr><td colspan="6" class="px-5 py-12 text-center text-slate-500">Tidak ada riwayat pembayaran.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
