...
<div x-data x-show="$store.bayarModal.show" x-cloak
    ...
    <div class="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl border border-slate-200 bg-white p-8 shadow-2xl [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
        ...
        <div class="flex items-center justify-between mb-6">
            <h3 class="text-xl font-bold text-slate-800" x-text="$store.bayarModal.isEdit ? 'Edit Pembayaran' : 'Tambah Pembayaran'"></h3>
            <button @click="$store.bayarModal.close()" class="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-800 transition-colors">
        ...
        <form :action="$store.bayarModal.formAction" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-slate-600">Metode</label>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-slate-600">Nominal</label>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-6 pt-1">
                        <label class="flex items-center gap-2 cursor-pointer group">
                            ...
                            <span class="text-sm text-slate-600 group-hover:text-slate-800 transition-colors">Valid</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer group">
                            <input type="radio" name="status" value="invalid" x-model="$store.bayarModal.form.status"
                                   class="h-4 w-4 border border-slate-300 bg-transparent text-red-600 focus:ring-red-500 focus:ring-offset-0 focus:ring-offset-transparent">
                            <span class="text-sm text-slate-600 group-hover:text-slate-800 transition-colors">Invalid</span>
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div>
                <label class="mb-1.5 block text-sm font-medium text-slate-600">Keterangan</label>
            ...
            <div x-data="{ photoPreview: null }">
                <label class="group relative flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 p-6 overflow-hidden cursor-pointer hover:border-red-500 hover:bg-slate-100 transition-all duration-200">
                    ...
                    <div x-show="!photoPreview" class="flex flex-col items-center">
                        <svg class="h-8 w-8 text-slate-400 group-hover:text-red-500 transition-colors mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m6.75 12l-3-3m0 0l-3 3m3-3v6m-1.5-15H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/>
                        </svg>
                        <span class="text-sm font-medium text-slate-500 group-hover:text-slate-700 transition-colors">Upload Bukti Pembayaran</span>
...
function bayarPage() {
    return {
        ...
        hapus(id) {
            Swal.fire({ title:'Hapus pembayaran?', text:'Data pembayaran ini akan dihapus.', icon:'warning', showCancelButton:true, confirmButtonColor:'#EF4444', cancelButtonColor:'#64748B', confirmButtonText:'Ya, Hapus!', cancelButtonText:'Batal', background:'#ffffff', color:'#1e293b' }).then(r => {
